...
This file is modified from poster project.
...
def encode_and_quote(data): # return str
...
class MultipartParam(object):
    ...
    def __init__(self, name, value=None, filename=None, filetype=None,
        ...
        self.name = Header(name).encode() # str
        self.value = value if isinstance(value, bytes) or value is None else _strify(value).encode() # bytes
        if filename is None: # str
            self.filename = None
        else:
            if isinstance(filename, str):
                # Encode with XML entities
                self.filename = filename.encode("ascii", "xmlcharrefreplace").decode()
            else:
                self.filename = str(filename)
            #self.filename = self.filename.replace('"', '\\"')
        self.filetype = _strify(filetype) #str
    ...
    def encode_hdr(self, boundary): # return bytes
    ...
    def encode(self, boundary): # return bytes
        ...
        if ('--' + encode_and_quote(boundary)).encode() in value:
            raise ValueError("boundary found in encoded string")

        return self.encode_hdr(boundary) + value + '\r\n'.encode()

    def iter_encode(self, boundary, blocksize=4096): # yield bytes
        ...
        if self.value is not None:
            ...
            if self.cb:
                self.cb(self, current, total)
            last_block = b''
            encoded_boundary = ("--" + encode_and_quote(boundary)).encode()
            while True:
                ...
                if not block:
                    ...
                    break
                last_block += block
                if encoded_boundary in last_block:
...
def get_body_size(params, boundary):
    ...
    return size + len(boundary.encode()) + 6
